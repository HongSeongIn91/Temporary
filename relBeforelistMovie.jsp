		...
		<c:if test="${moviePage.hasMovies() }">
			<tr>
				<td colspan="4"><c:if test="${moviePage.startPage > 5 }">
						<a href="movierelbefore.do.do?pageNo=${moviePage.startPage - 5 }&movieSort=${movieSort}">[이전]</a>
					</c:if> <c:forEach var="pNo" begin="${moviePage.startPage }"
						end="${moviePage.endPage }">
						<a href="movierelbefore.do.do?pageNo=${pNo }&movieSort=${movieSort}">[${pNo }]</a>
					</c:forEach> <c:if test="${moviePage.endPage < moviePage.totalPages }">
						<a href="movierelbefore.do.do?pageNo=${moviePage.startPage + 5 }&movieSort=${movieSort}">[다음]</a>
					</c:if></td>
			</tr>
		</c:if>
	</table>
