...
<tr>
	...
	<c:set var="pageNo" value="${empty param.pageNo ? '1' : param.pageNo }"></c:set>
		...
		<c:if test="${authUser.id == reviewData.review.rv_m_id }">
			<a href="modify.do?rv_num=${reviewData.review.rv_num }">[게시글 수정]</a>
			<a href="delete.do?rv_num=${reviewData.review.rv_num }">[게시글 삭제]</a>
		</c:if>
	</td>
</tr>
</table>
		...
		<c:forEach var="reply" items="${replyPage.content}">
			<tr>
				...
				<c:if test="${authUser.id == reply.rp_m_id}">
					<td>
						<div>
							<a href="modifyReply.do?rp_num=${reply.rp_num}&rp_rv_num=${reviewData.review.rv_num}&replypageNo=${replyPage.currentPage}">
								<input type="button" value="수정">
							</a>
						<p>
							<a href="deleteReply.do?rp_num=${reply.rp_num}&rp_rv_num=${reviewData.review.rv_num}&replypageNo=${replyPage.currentPage}">
		...
		<c:if test="${replyPage.hasReplies() }">
			<tr>
				<td colspan="4"><c:if test="${replyPage.startPage > 5}">
						<a
							href="read.do?replypageNo=${replyPage.startPage - 5}&no=${reviewData.review.rv_num }">[이전]</a>
					</c:if> <c:forEach var="pNo" begin="${replyPage.startPage }"
						end="${replyPage.endPage }">
						<a
							href="read.do?replypageNo=${pNo}&no=${reviewData.review.rv_num }">[${pNo }]</a>
					</c:forEach> <c:if test="${replyPage.endPage < replyPage.totalPages }">
						<a
							href="read.do?replypageNo=${replyPage.startPage + 5}&no=${reviewData.review.rv_num }">[다음]</a>
					</c:if></td>
			</tr>
		</c:if>
	</table>
